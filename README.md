...
5. **Set Up the Database**
    ...
    Ensure PostgreSQL is running and set up the necessary tables by running the following command:
...
8. **Open 'index.html' or Running Local Host**

    ```bash
    open index.html or navigate to http://localhost:5000
    ```
...
- 🖥️ **JavaScript/TypeScript**: Interactivity, API integration, and TypeScript for type safety
- 🤖 **Google Generative Language API**: Backend service to generate AI responses
- ⚙️ **Node.js**: Server-side environment
- 📦 **Express.js**: Backend web framework
- 🗄️ **PostgreSQL**: Database for storing chat messages
...
- **Typing Speed**: Adjust the typing effect speed by modifying the interval in `showTypingEffect` function within `ui.js`.

## Troubleshooting

- **🔍 Blank Screen**: Ensure that the API key is correctly set and that the browser's console does not show any errors.
- **⚠️ Chat Not Loading**: Check if the images in the `img` folder are correctly referenced and that the API is returning valid responses.
...
│   ├── api.ts             # API interactions and server-side logic
...
│   │   ├── core.ts        # Core logic and global state management
│   │   ├── ui.ts          # UI manipulation functions
│   │   ├── events.ts      # Event listeners and handlers
│   │   └── main.ts        # Entry point for frontend scripts
│   ├── css/               # CSS files
